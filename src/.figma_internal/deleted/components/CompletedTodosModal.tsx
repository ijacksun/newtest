...
export function CompletedTodosModal({ isOpen, onClose, completedTodos, onDeleteCompleted, onRestoreCompleted }: CompletedTodosModalProps) {
  if (!isOpen) return null;

  // Garantir que completedTodos não seja undefined
  const safeTodos = completedTodos || [];
        ...
        {safeTodos.length > 0 && (
          <div className="border-t border-gray-200 p-4 bg-gray-50">
            <p className="text-xs text-gray-500 text-center">
              {safeTodos.length} tarefa{safeTodos.length !== 1 ? 's' : ''} concluída{safeTodos.length !== 1 ? 's' : ''}
            </p>
          </div>
        )}
      </div>
